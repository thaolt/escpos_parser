...
const unsigned char* te_data = NULL;
...
int caculate_cmd_data_length(escpos_cmd_t** cmd, const unsigned char* data);
...
void escpos_cmd_declare(escpos_cmd_specs_t** cs, char* signature, char* id,
                        unsigned int fixed_length, bool nul_terminated, char* description,
                        char* lcexpr)
{
...
unsigned int escpos_parse(escpos_vec_token_t* token_list, escpos_cmd_specs_t** cs,
                          const unsigned char* data, unsigned int datalen)
{
    ...
    for (size_t i = 0; i < datalen; i++)
    {
        ...
        if (c <= 0x1F)
        {
            ...
            if (found_node != NULL)
            {
                ...
                if (found_node != NULL)
                {
                    ...
                    {
                        last_token->data_len =
                            caculate_cmd_data_length(&(last_token->cmd), &(data[i + 1]));
                    }
        ...
        { // cmd not found, data instead
            if (last_token && last_token->cmd == NULL)
            {
                last_token->data_len++;
                last_token->data =
                    (unsigned char*)realloc(last_token->data, last_token->data_len + 1);
            }
            else
            {
                last_token = (escpos_token_t*)calloc(1, sizeof(escpos_token_t));
...
int caculate_cmd_data_length(escpos_cmd_t** cmd, const unsigned char* data)
{
    ...
    int err;
    unsigned int ret = 0;
    te_variable vars[] = {{"d", _escpos_lcexpr_data_func, TE_FUNCTION1}};
